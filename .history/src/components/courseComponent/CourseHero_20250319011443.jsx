...
const CourseHero = ({ course }) => {
  ...
  const signUp = (e) => {
    ...
    const phoneRegex = /^01[3-9]\d{8}$/; // Validates Bangladeshi phone numbers starting with 01 followed by a digit between 3 and 9 and then 8 more digits
    if (!phoneRegex.test(number)) {
      ...
      // Trigger the modal hide action by setting the data attribute (if modal uses this approach)
      const modal = document.getElementById("authentication-modal");
      if (modal) {
        // Add 'hidden' class to close the modal
        modal.classList.add("hidden");

        // Alternatively, you can use `setAttribute` to trigger the modal's data attribute handler if it's handled by a library
        modal.setAttribute("data-modal-hide", "authentication-modal");
      }
  ...
  return (
    <div>
      ...
      {isOpen && (
        <div
          className="pointer-events-auto fixed inset-0 z-[999] grid h-screen w-screen place-items-center bg-black bg-opacity-90 opacity-100 backdrop-blur-sm transition-opacity duration-300"
          onClick={closeModal}
        >
          <div
            className="relative m-4 p-4 w-2/5 min-w-[20%] max-w-[20%] rounded-lg bg-white shadow-sm"
