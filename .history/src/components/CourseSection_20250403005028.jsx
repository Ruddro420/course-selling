...
const CourseSection = ({ courses }) => {
  ...
  // load context
  const { purchasedCourses,isCoursePurchased } = useContext(DataContext);
          ...
          <div className="bg-white p-6 mt-4 rounded-b-lg">
            ...
            {isCoursePurchased(course.course_name) ? (
              <Link to={`/course-content/${course.id}`} className="p-0 cursor-pointer">
                <button className="w-full cursor-pointer bg-green-600 text-white py-3 rounded-lg font-semibold hover:bg-green-700 transition duration-300 mt-4">
                  View Course
                </button>
              </Link>
            ) : (
