...
import Bn from "../components/EnToBn/Bn";
...
const Checkout = () => {
    ...
    // Load context
    const { getdataApiData } = useContext(DataContext);

    useEffect(() => {
        if (getdataApiData?.data) {
            const finddata = getdataApiData.data.find(item => item.id === parseInt(id));
            setData(finddata);
        }
    }, [id, getdataApiData]);
